...
"""
import pytest
from digest_core.evidence.actions import ActionMentionExtractor, ExtractedAction, enrich_actions_with_evidence
...
class TestConfidenceScoring:
    ...
    def test_high_confidence_all_signals(self, extractor):
        ...
        actions = extractor.extract_mentions_actions(text, "msg-017", "important@corp.com", sender_rank=0.9)
    ...
    def test_confidence_with_sender_rank(self, extractor):
        ...
        # Low sender rank
        actions_low = extractor.extract_mentions_actions(text, "msg-020", "junior@corp.com", sender_rank=0.1)
        
        # High sender rank
        actions_high = extractor.extract_mentions_actions(text, "msg-021", "ceo@corp.com", sender_rank=0.9)
...
class TestGoldSetValidation:
    ...
    def test_gold_set_precision_recall(self, extractor):
        ...
        for text, should_extract, expected_type, has_deadline in self.GOLD_SET:
            actions = extractor.extract_mentions_actions(text, f"msg-gold-{hash(text)}", "test@corp.com")
